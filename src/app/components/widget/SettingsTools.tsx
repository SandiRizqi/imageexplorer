...
const SettingsTools: React.FC<AOISettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      ...
      >
        <DialogTitle className="text-lg font-bold text-greensecondarycolor">{t.settingsTitle}</DialogTitle>
        <p className="text-sm my-2">{t.customizeAOI}</p>

        {/* Two-column layout */}
        <div className="grid grid-cols-2 gap-4 mt-4">
          {/* Left Column: Color Setting */}
          <div>
            <label className="block text-xs">{t.aoiColor}</label>
            <input
              type="color"
              value={color}
          ...
          <div>
            <label className="block text-xs">{t.borderWidth}</label>
            <input
              type="number"
              value={borderWidth}
              min={1}
        ...
        <div className="flex justify-end mt-4 space-x-2">
          <button onClick={handleSave} className="bg-greensecondarycolor px-3 py-1 text-white rounded-sm">{t.save}</button>
        </div>
      </DialogPanel>
    </Dialog>
  );
