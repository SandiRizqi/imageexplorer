...
export default function Cart({ isMobile }: CartProps) {
    ...
    .map((item) => {
      ...
      if (turf.booleanIntersects(imagePolygon, regionPolygon)) {
        const intersection = turf.intersect(
          turf.featureCollection([imagePolygon, regionPolygon])
        );
        if (intersection) {
      ...
      const coverage =
        intersectionArea > 0 ? (intersectionArea / polyArea) * 100 : 0;
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      const savedConfigID = await saveConfig(
        configData,
        setError,
        config.configID
      );
                  ...
                  {cartItem.length > 0 ? (
                    <ul>
                      {cartItem.map((item, index) => (
                        <li
                          key={index}
                          className="p-3 mt-1 border-b border-gray-700 bg-maincolor rounded-lg flex flex-col text-xs"
                        >
                          {/* Details */}
                          <p className="font-semibold text-white truncate w-50">
                  ...
                  >
                    {!loading ? (
                      ...
                      <div className="flex items-center">
                        <svg
                          className="animate-spin h-4 w-4 text-gray-900 mr-2"
                          viewBox="0 0 24 24"
                          fill="none"
